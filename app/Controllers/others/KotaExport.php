...
class KotaExport extends BaseController
{
    public function __construct()
    {
        ...
        $this->KategoriModel = new ProvinsiModel();
    }

    function export_xls()
    {
        //select data from table buku
        $list = $this->KotaModel->select('kota.id, kota.nama, provinsi.nama AS provinsi_nama')->join('provinsi', 'kota.provinsi_id = provinsi.id')->orderBy('provinsi.nama')->findAll();
        ...
        $sheet->setCellValue('C1', 'Nama Kota')->getColumnDimension('C')->setAutoSize(true);
